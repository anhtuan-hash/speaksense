...
**Goal:** Coordinate the five independently testable implementation plans into one research-ready SpeakSense release without losing the October 2026 competition deadline.

**Architecture:** Foundation/security is the base dependency. Acoustic feasibility is an early hard gate and should begin as soon as the web scaffold exists. Audio/offline and model work can overlap after the foundation interfaces stabilize; student research flows follow scoring; teacher/research analytics close the release.
...
2. `2026-09-15-02-audio-offline-storage.md` — recording, preprocessing, private storage, offline queue.
3. `2026-09-15-03-acoustic-alignment-scoring.md` — early model benchmark hard gate, alignment, six scores, confidence/error model.
4. `2026-09-15-04-student-practice-adaptive-test.md` — three Labs, feedback, adaptive practice, pre/post and cohort enforcement.
5. `2026-09-15-05-teacher-validation-research-admin.md` — analytics, blind validation, exports, retention/admin, full pilot gate.

## Dependency and parallelization rules

- [ ] Complete Plan 01 Tasks 1–4 before any feature depends on Supabase tables/RLS.
- [ ] Start Plan 03 Task 1 (model benchmark) immediately after the Vite scaffold exists; this is the highest technical-risk gate and may run in parallel with remaining Plan 01 work.
- [ ] Plan 02 may start once `attempts` schema and auth identity contracts are stable.
...
- [ ] Plan 04 depends on stable `ScoreResult`, `SyncQueue`, class/study/cohort records.
- [ ] Plan 05 analytics can begin with fixture data after schemas are stable, but the final pilot gate depends on Plans 01–04 being complete.
...
- [ ] US/UK references used by the pre-test are versioned;
...
- `v0.4-student-research` after Plan 04.
